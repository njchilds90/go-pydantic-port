...
// AddCustomValidator registers a model-scoped custom validator.
func (m *Model) AddCustomValidator(name string, fn func(value any) error) *Model {
	return m.AddValidator(name, func(_ context.Context, value any) error { return fn(value) })
}
...
// WithStrictMode toggles strict mode for this model.
func (m *Model) WithStrictMode(strict bool) *Model {
	return m.SetStrictMode(strict)
}
...
func (m *Model) Field(name string, spec any, rules ...string) *FieldBuilder {
	...
	for _, r := range rules {
		...
		for _, parsed := range rs {
			...
			case "items":
				f.Type = "array"
				f.Items = &Field{Name: name + "_item", Type: parsed.Arg}
			case "custom":
				if parsed.Arg != "" {
					f.Custom = append(f.Custom, parsed.Arg)
				}
			default:
